...
const programData = {
  ...
  dy_sobang: {
    icon: '📸', name: '대영소방 현장기록 (DY-Sobang FieldLog)', tag: '현장기록 · 안드로이드',
    desc: '소방 점검 현장에서 사진 + 음성 메모를 즉시 기록하는 안드로이드 앱. GPS·주소·시각·메모·사업장이 사진에 워터마크로 자동 합성되어, 사진 한 장이 그 자체로 점검 보고서 역할을 합니다. 한국어 음성인식(STT)으로 받아쓰기 후 카톡·드라이브·메일로 즉시 공유. 자동 업데이트 시스템 내장 — 새 버전 출시 시 폰에서 알림 → 위에 덮어쓰기 설치로 IndexedDB 기록 유지.',
    features: [
