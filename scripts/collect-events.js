...
// - 매일 1회 cron 실행
// - travel(여행 특가) / giveaways(공모·응모) / coffee(커피 경품) 수집
...
const TOPICS = [
  {
    ...
    kindLabel: '🎁 여행 특가',
    queries: [
      { ...KO, q: '"여행 특가" OR "항공권 특가"' },
      { ...KO, q: '"호텔 할인" OR "패키지 여행 할인" OR "해외여행 프로모션"' }
    ]
  },
  {
    key: 'giveaways',
...
function normalize({ raw, region, forcedSource }, topic) {
  ...
  if (topic.key === 'travel') {
    return { kind: topic.kindLabel, title, detail: source, url: link, site: 'gnews_' + source, created_at: dateStr };
  }
  if (topic.key === 'coffee') {
    return {
      source_site: source,
...
// 같은 사건을 여러 매체가 조금씩 다르게 쓴 제목("…재개 '최대 300만원 혜택'" /
// "…7월까지 최대 300만원 경품 추첨")은 정확 일치 키로는 안 잡힌다.
// 제목 토큰의 자카드 유사도로 근접 중복을 걸러낸다.
...
function isNearDuplicate(tokens, seenTokenSets) {
  return seenTokenSets.some(prev => jaccard(tokens, prev) >= NEAR_DUP_THRESHOLD);
}
...
async function main() {
  ...
  for (const topic of TOPICS) {
    existing[topic.key] = pruneOld(existing[topic.key]).slice(0, MAX_PER_CATEGORY);
  }
  const now = new Date();
